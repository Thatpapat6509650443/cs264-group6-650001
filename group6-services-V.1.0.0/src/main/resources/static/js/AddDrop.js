...
document.addEventListener("DOMContentLoaded", function() {
  ...
  submitButton.addEventListener("click", function(event) {
      if (dateField.value.trim() === "") {
          // เปลี่ยนสีขอบของ input เป็นสีแดง
          dateField.style.border = "2px solid red";

          event.preventDefault(); // ยกเลิกการรีเฟรชหน้าเว็บ

          // เลื่อนหน้าเว็บไปยัง input field ที่ผิดพลาด
          dateField.scrollIntoView({ behavior: "smooth" });

          // เพิ่ม alert เพื่อแจ้งเตือนผู้ใช้
          alert("กรุณากรอกวันที่");
      } else {
          // ถ้าไม่มีข้อผิดพลาด ก็ล้างสีขอบเพื่อไม่มีขอบ
          dateField.style.border = "none";
      }
  });

  dateField.addEventListener("input", function() {
      // ล้างสีขอบเมื่อผู้ใช้ป้อนข้อมูลใหม่
      dateField.style.border = "none";
  });
});
